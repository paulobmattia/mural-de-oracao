...
function Header({ view, setView, activeWall, goBack, onLeaveClick }) {
  ...
  return (
    <div className="bg-[#973130] shadow-sm p-4 sticky top-0 z-20 flex items-center justify-between text-white h-16 transition-colors duration-300">
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto pt-8 animate-in fade-in">
      <div className="text-center mb-8">
        ...
        <div className="bg-[#973130]/10 dark:bg-[#973130]/20 p-4 rounded-xl inline-block max-w-md shadow-sm border border-[#973130]/20 dark:border-[#973130]/30">
      ...
      {loading ? (<div className="flex justify-center p-8"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#973130]"></div></div>) : myWalls.length === 0 ? (
        ...
        <div className="space-y-3">{myWalls.map(wall => (<button key={wall.id} onClick={() => onSelectWall(wall)} className="w-full bg-white dark:bg-slate-800 p-5 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 flex items-center justify-between hover:shadow-md transition-all text-left group"><div className="flex items-center gap-4"><div className="w-12 h-12 rounded-full bg-[#973130]/10 dark:bg-[#973130]/30 text-[#973130] dark:text-[#bc5c5b] flex items-center justify-center font-bold text-lg">{wall.title.charAt(0).toUpperCase()}</div><div><h4 className="font-bold text-slate-800 dark:text-white text-lg group-hover:text-[#973130] dark:group-hover:text-[#bc5c5b] transition-colors">{wall.title}</h4><p className="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1"><Users size={12} /> {wall.memberCount || 1} intercessores</p></div></div><div className="text-slate-300 group-hover:translate-x-1 transition-transform"><ArrowLeft size={20} className="rotate-180" /></div></button>))}</div>
      )}
    </div>
  );
}
