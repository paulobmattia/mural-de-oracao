...
import { 
  ...
  Calendar, Bell, Moon, Sun, Camera, Users, KeyRound, Search, LogIn
} from 'lucide-react';
...
const appId = "mural-v2"; // Mudamos para v2 para separar os dados novos

// --- Componente Principal ---
export default function PrayerApp() {
  const [user, setUser] = useState(null);
  const [userProfile, setUserProfile] = useState(null); 
  const [view, setView] = useState('splash'); // splash, login, wall-list, wall-detail, create-wall, join-wall, settings
  const [activeWall, setActiveWall] = useState(null); // O mural que está aberto no momento
  const [theme, setTheme] = useState(() => localStorage.getItem('theme') || 'light');

  // Configuração Inicial (Tema e Favicon)
  useEffect(() => {
          ...
          if (docSnap.exists()) {
            ...
            setView(v => (v === 'splash' || v === 'login' ? 'wall-list' : v));
             ...
             const initialData = { 
                ...
                joinedWalls: [] // Lista de IDs dos murais que participa
             };
  ...
  const handleCreateWall = async (title, password) => {
    ...
    try {
      ...
      const wallRef = await addDoc(collection(db, 'artifacts', appId, 'prayer_walls'), {
        title: title.trim(),
        password: password.trim(), // Em app real, isso deveria ser hash
        createdBy: user.uid,
        createdAt: serverTimestamp(),
        memberCount: 1
      });

      // 2. Adicionar ao perfil do usuário
      const userRef = doc(db, 'artifacts', appId, 'users', user.uid, 'profile', 'main');
      await updateDoc(userRef, {
        joinedWalls: arrayUnion(wallRef.id)
      });

      // 3. Atualizar estado local e entrar
  ...
  const handleJoinWall = async (nameSearch, passwordInput) => {
    ...
    try {
      // Buscar mural por nome exato e senha
      const q = query(
      ...
      // Atualizar contagem de membros no mural
...
function WallListScreen({ userProfile, db, appId, onSelectWall, onCreateNew, onJoinExisting }) {
  ...
  useEffect(() => {
    const fetchWalls = async () => {
      ...
      // Firestore não permite buscar muitos IDs de uma vez, então buscamos um por um ou usamos chunks
      // Como esperamos poucos murais por usuário, Promise.all é ok
      try {
        ...
        <div className="space-y-3">
          {myWalls.map(wall => (
            <button 
              ...
              className="w-full bg-white dark:bg-slate-800 p-5 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 flex items-center justify-between hover:shadow-md transition-all text-left group"
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1">
                    <Users size={12} /> {wall.memberCount || 1} intercessores
                  </p>
                </div>
              </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <input required type="text" placeholder="Ex: Família Silva, Célula Jovem..." value={title} onChange={e => setTitle(e.target.value)} className="w-full p-4 pl-12 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-blue-500 dark:text-white" />
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <input required type="text" placeholder="Defina uma senha para convidados" value={password} onChange={e => setPassword(e.target.value)} className="w-full p-4 pl-12 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-blue-500 dark:text-white" />
          </div>
        ...
        <div className="flex gap-3 mt-4">
          ...
          <button type="submit" disabled={loading} className="flex-1 bg-blue-600 text-white p-4 rounded-xl font-bold hover:bg-blue-700 transition-colors shadow-lg shadow-blue-200 dark:shadow-none">
    ...
    <div className="p-6 max-w-md mx-auto pt-10 animate-in fade-in slide-in-from-bottom-4">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            <KeyRound className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={20} />
            <input required type="text" placeholder="Digite a senha de acesso" value={password} onChange={e => setPassword(e.target.value)} className="w-full p-4 pl-12 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-purple-500 dark:text-white" />
          </div>
        </div>

        <div className="flex gap-3 mt-4">
          <button type="button" onClick={onCancel} className="flex-1 p-4 rounded-xl font-bold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 transition-colors">Cancelar</button>
          <button type="submit" disabled={loading} className="flex-1 bg-purple-600 text-white p-4 rounded-xl font-bold hover:bg-purple-700 transition-colors shadow-lg shadow-purple-200 dark:shadow-none">
...
function WallDetailScreen({ wall, user, userProfile, db, appId, onLeaveWall }) {
  ...
  // Listener de Pedidos (AGORA FILTRADO PELO ID DO MURAL)
  useEffect(() => {
    ...
    <div className="pb-20">
      ...
      <div className="bg-white dark:bg-slate-800 p-4 sticky top-[64px] z-10 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between shadow-sm">
        ...
        <div className="flex gap-2">
          <button onClick={onLeaveWall} className="p-2 text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-full" title="Sair do Mural">
            <LogOut size={20} />
          </button>
        </div>
      </div>
      ...
      <div className="flex p-4 gap-2 justify-center">
        <button 
          ...
          className={`flex-1 py-2 rounded-lg font-bold text-sm transition-colors ${mode === 'read' ? 'bg-blue-100 text-blue-600 dark:bg-blue-900 dark:text-blue-300' : 'bg-transparent text-slate-400'}`}
        >
          Mural
        </button>
        <button 
          onClick={() => setMode('write')} 
          className={`flex-1 py-2 rounded-lg font-bold text-sm transition-colors ${mode === 'write' ? 'bg-blue-100 text-blue-600 dark:bg-blue-900 dark:text-blue-300' : 'bg-transparent text-slate-400'}`}
        >
          Novo Pedido
        </button>
      </div>
      ...
      <div className="px-4">
        {mode === 'write' ? (
          ...
          <ReadScreen 
            ...
            wallId={wall.id} // Passar ID para comentários
            appId={appId}
            db={db}
          />
        )}
      ...
      {/* Modal Exclusão de Pedido (Local) */}
      {deleteModal.isOpen && (
...
// --- COMPONENTES DE LEITURA/ESCRITA (Adaptados para receber props extras) ---
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pb-20 animate-in fade-in duration-500">
      {requests.map((req) => (
        <PrayerCard 
          ...
          wallId={wallId} // Importante para subcoleção
          appId={appId}
          db={db}
        />
      ))}
...
// --- OUTROS COMPONENTES (Mantidos da versão anterior, apenas reutilizados) ---
// Avatar, Header, SettingsScreen, LoginScreen (com injeção de props)
// (O código completo acima já inclui as versões adaptadas para esta nova arquitetura)
...
function LoginScreen({ onLoginSuccess, appId, db, auth }) {
  // Mesma lógica de login, apenas chamando onLoginSuccess ao final
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-white dark:bg-slate-900 animate-in fade-in slide-in-from-bottom-8 duration-700 transition-colors duration-300">
      ...
      <button onClick={handleGoogleLogin} className="w-full max-w-sm bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 p-4 rounded-xl font-bold hover:bg-slate-50 dark:hover:bg-slate-700 active:scale-95 transition-all flex items-center justify-center gap-3">
