    ...
    <div className="pb-20 pt-4 relative h-full">
      <div className="mb-4 flex flex-col gap-2">
        ...
        {!showTestimonials && (
          <div className="-mx-6 px-6 flex gap-2 overflow-x-auto pb-2 no-scrollbar w-[calc(100%+48px)]">
             {CATEGORIES.map(tag => (
               <button key={tag} onClick={() => setFilterTag(filterTag === tag ? null : tag)} className={`px-3 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-colors border flex-shrink-0 ${filterTag === tag ? 'bg-blue-100 text-blue-600 border-blue-200 dark:bg-blue-900 dark:text-blue-300 dark:border-blue-800' : 'bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-700'}`}>{tag}</button>
             ))}
             <div className="w-4 flex-shrink-0"></div>
          </div>
        )}
      ...
      <div className="px-4">
        {mode === 'write' ? (
          ...
          <ReadScreen requests={filteredRequests} loading={loading} currentUser={user} userProfile={userProfile} onPray={handlePray} onDeleteClick={(id) => setDeleteModal({ isOpen: true, requestId: id })} onMarkAnswered={(id) => setMarkAnsweredModal({ isOpen: true, requestId: id })} wallId={wall.id} appId={appId} db={db} isWallAdmin={isWallAdmin} isTestimonialMode={showTestimonials} />
        )}
      </div>
