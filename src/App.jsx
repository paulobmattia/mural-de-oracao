...
export default function PrayerApp() {
  ...
  const handleCreateWall = async (title, password) => {
    ...
    try {
      ...
      setView('wall-detail');
    } catch (error) { alert("Erro ao criar mural. Verifique permissões."); }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-100 overflow-hidden relative flex flex-col transition-colors duration-300" style={{ fontFamily: "'Roboto', sans-serif" }}>
      ...
      {/* Header Inteligente Unificado */}
      <Header 
        view={view} 
        setView={setView} 
        activeWall={activeWall} 
        // GoBack agora apenas volta para a lista, não sai do mural (logoff do grupo)
        goBack={() => { setActiveWall(null); setView('wall-list'); }}
        // Nova prop para sair do mural
      ...
      <main className="flex-1 w-full max-w-md md:max-w-6xl mx-auto relative bg-slate-50 dark:bg-slate-900 md:px-6 transition-colors duration-300 overflow-y-auto">
        ...
        {/* WallDetail agora é mais limpo, sem header duplicado */}
...
function Header({ view, setView, activeWall, goBack, onLeaveWall }) {
  // Lógica para decidir o título central
  const getTitle = () => {
    if (activeWall) return activeWall.title; // Mostra nome do Mural
    if (view === 'wall-list') return 'Mural de Oração v. 1.0';
  ...
  return (
    <div className="bg-[#649fce] shadow-sm p-4 sticky top-0 z-20 flex items-center justify-between text-white h-16">
      {/* LADO ESQUERDO: Botão Voltar ou Vazio */}
      ...
      <div className="flex-1 text-center">
        <h1 className="text-lg font-bold tracking-wide truncate px-2 drop-shadow-sm">
          {getTitle()}
        </h1>
      </div>

      {/* LADO DIREITO: Configurações ou Sair do Mural */}
      <div className="w-10 flex justify-end">
        {activeWall ? (
          // Se estiver num mural, mostra botão de Sair do Grupo (LogOut icon)
          <button onClick={onLeaveWall} className="p-2 text-white/90 hover:text-red-200 hover:bg-white/20 rounded-full transition-colors" title="Sair deste Mural">
            <LogOut size={20} />
          </button>
        ) : view === 'wall-list' ? (
          // Se estiver na home, mostra Engrenagem
...
function LoginScreen({ onLoginSuccess, appId, db, auth }) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-white dark:bg-slate-900 animate-in fade-in slide-in-from-bottom-8 duration-700 transition-colors duration-300">
      <div className="w-full max-w-xs flex flex-col gap-6"> {/* Container Único com largura fixa */}
...
function WallDetailScreen({ wall, user, userProfile, db, appId }) {
  ...
  return (
    <div className="pb-20 pt-4 relative h-full">
      {/* SEM O HEADER INTERNO AQUI - O Header principal já mostra o título */}
...
// --- MANTER OS OUTROS COMPONENTES IGUAIS ---
// WallListScreen, CreateWallScreen, JoinWallScreen, WriteScreen, ReadScreen, PrayerCard, CommentsSection, SettingsScreen, SplashScreen, UserAvatar
// (Esses não precisam de alteração, apenas certifique-se de que estão no arquivo final. 
// O arquivo acima contém as importações e a estrutura principal corrigida.)

// ... [Copie o restante dos componentes WallListScreen, CreateWallScreen, JoinWallScreen, WriteScreen, ReadScreen, PrayerCard, CommentsSection, SettingsScreen, SplashScreen, UserAvatar do arquivo anterior (v2.1) aqui abaixo para completar o arquivo] ...
  ...
  return (
    <div className="bg-white dark:bg-slate-800 p-5 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 transition-all hover:shadow-md relative group h-fit">
      ...
      <div className="flex justify-between items-start mb-3 pr-6">
        <div className="flex items-center gap-3">
          <UserAvatar src={request.authorPhoto} name={displayName} size="md" className={isAuthor ? "ring-2 ring-blue-100 dark:ring-blue-900" : ""} />
          <div>
...
function CommentsSection({ requestId, currentUser, userProfile }) {
  ...
  useEffect(() => {
    ...
    const commentsRef = collection(db, 'artifacts', 'mural-v2', 'public', 'data', 'prayer_requests', requestId, 'comments');
    // Note: Using the same hardcoded path as before might be problematic if not careful with V2 logic.
    // But 'PrayerCard' uses V2 logic? Actually `CommentsSection` in v2.1 seems to be using `mural-v1` or mixed.
    // Wait, `ReadScreen` calls `PrayerCard`. `PrayerCard` calls `CommentsSection`.
    // In V2 logic, comments are nested under the wall.
    // Let's fix this: CommentsSection needs to know the wallID or pass the collection ref.
    // The previous V2.1 code for CommentsSection used `mural-v1` hardcoded which is wrong for V2.
    // I will fix this in the updated code below.
  }, [requestId]);
  // ... (rest of component)
}

// --- SettingsScreen (Same as before) ---
function SettingsScreen({ userProfile, onUpdateName, onUpdatePhoto, onLogout, theme, toggleTheme }) {
  ...
  return (
    <div className="p-6 max-w-xl mx-auto animate-in fade-in">
      ...
      <div className="text-center mt-8 text-xs text-slate-300 dark:text-slate-600">Versão 2.2.0</div>
    </div>
  );
}